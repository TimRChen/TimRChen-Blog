...
Vue.use(httpInterceptor, {
  operation: (...args) => {
    ...
    request.headers.map.Authorization = [window.localStorage.getItem('Authorization') || '']
    next((response) => {
      if (typeof response.headers.map.Authorization === 'object') {
        window.localStorage.setItem('Authorization', response.headers.map.Authorization[0])
